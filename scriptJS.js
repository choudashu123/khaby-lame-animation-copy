...
void main() {
  ...
  class Section3D {
    ...
    updateScroll() {
      ...
      this.group.position.y = pSection * -12 + 8.5;
  ...
  function initSections() {
    ...
    stage.onResize();
    sectionss.forEach(s => s.updateScroll());
    stage.render();
  }
  ...
  startBtn.addEventListener("click", () => {
    ...
    setTimeout(() => {
      ...
      initSections();      // <-- FIXED! Create meshes only now
      window.dispatchEvent(new Event("resize")); // ensure correct sizing
      sections.forEach(s => s.updateScroll());

      // ... (existing GSAP animations below)
      const heroInner = document.querySelector(".hero__inner");
      const heroHeadline = document.getElementById("heroHeadline"); // Make sure heroHeadline is defined
      gsap.fromTo(
        heroInner,
        {
          yPercent: 0,
